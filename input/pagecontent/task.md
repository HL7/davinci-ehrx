...
§task-1?^poll-imp^exchange:For Da Vinci, systems that use polling **SHALL** check for new/updated information at least once per business day and **SHOULD** check for information at least once per hour during
§normal hours of operation.  §task-2?^poll-imp^exchange:Polling systems **SHOULD NOT** query more often than every 15 minutes unless there is an urgent change they are monitoring for.§
...
support.  §task-3?^subscribe-imp^exchange:Implementers of this Da Vinci IG who choose to support Subscription **SHALL** comply with the Subscription Backport IG for the purpose of monitoring Tasks.§
...
§task-4?^subscribe-imp^exchange:Systems supporting subscription **SHALL** support the rest-hook channel mechanism, though they might choose to support other channel approaches.§  §task-5?^subscribe-imp^exchange:Systems using subcription **SHALL** support id-only, though they can  also support other content approaches.§  The
